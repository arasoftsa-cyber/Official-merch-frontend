...
import { apiFetchForm, API_BASE } from '../../shared/api/http';
...
type ProductStatus = 'pending' | 'inactive' | 'active' | 'rejected';
type StatusFilter = 'all' | ProductStatus;
type SkuType = OnboardingSkuType;

type NewMerchFormState = {
  merchName: string;
  merchStory: string;
...
const SKU_OPTIONS: Array<{ value: SkuType; label: string; testId: string }> = [
  {
...
const ALLOWED_DESIGN_MIME_TYPES = new Set(['image/png', 'image/jpeg', 'image/jpg', 'image/svg+xml']);
const ALLOWED_DESIGN_EXTENSIONS = ['.png', '.jpg', '.jpeg', '.svg'];

const defaultSkuSelection = (): Record<SkuType, boolean> => ({
...
const initialNewMerchForm = (): NewMerchFormState => ({
  merchName: '',
  merchStory: '',
  designImage: null,
...
const normalizeProductStatus = (product: any): ProductStatus => {
...
const isProductActive = (product: any) => normalizeProductStatus(product) === 'active';

const isProductToggleable = (product: any) => {
  const status = normalizeProductStatus(product);
  return status === 'active' || status === 'inactive';
};

const readProductStory = (product: any) =>
  String(product?.merch_story ?? product?.merchStory ?? product?.description ?? '').trim();

const readProductDesignImageUrl = (product: any) =>
  String(product?.designImageUrl ?? product?.design_image_url ?? '').trim();

const readProductSkuTypes = (product: any): string[] => {
...
const readRejectionReason = (product: any) =>
  String(product?.rejectionReason ?? product?.rejection_reason ?? '').trim();

const formatStatusLabel = (status: ProductStatus) =>
  status.charAt(0).toUpperCase() + status.slice(1);

const getStatusPillClass = (status: ProductStatus) => {
  if (status === 'active') {
...
const getSelectedSkuTypes = (skuTypes: Record<SkuType, boolean>) =>
  SKU_OPTIONS.filter((entry) => skuTypes[entry.value]).map((entry) => entry.value);

const isAllowedDesignImage = (file: File) => {
...
const normalizeServerFieldMessage = (field: string, message: string) => {
...
const parseFormErrorMessage = (err: any) => {
